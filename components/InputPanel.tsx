...
function parseAuditData(raw: string): AuditData {
  ...
  if (!Array.isArray(parsed.categories) || parsed.categories.length !== 6) {
    throw new Error("`categories` must be an array of exactly 6 items (A–F).");
  }
  if (!Array.isArray(parsed.severeIssues) || parsed.severeIssues.length !== 5) {
    throw new Error("`severeIssues` must be an array of exactly 5 items.");
  }
  if (!Array.isArray(parsed.actionPlan) || parsed.actionPlan.length !== 3) {
    throw new Error("`actionPlan` must be an array of exactly 3 tiers.");
  }
